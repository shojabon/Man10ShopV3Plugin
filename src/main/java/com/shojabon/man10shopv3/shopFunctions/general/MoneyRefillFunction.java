...
@ShopFunctionDefinition(
        internalFunctionName = "moneyRefill",
        name = "時間ごとのお金自動補充設定",
        explanation = {},
        enabledShopType = {},
...
public class MoneyRefillFunction extends ShopFunction {
    ...
    //init
    public MoneyRefillFunction(Man10Shop shop, Man10ShopV3 plugin) {
        super(shop, plugin);
    }
    ...
    public String currentSettingString() {
        ...
        return getMinutes() + "分毎に" + getAmount() + "円補充";
    }
    ...
    public AutoScaledMenu getInnerSettingMenu(Player player, Man10ShopV3 plugin){
        AutoScaledMenu autoScaledMenu = new AutoScaledMenu("時間ごとのお金自動補充設定", plugin);
        ...
        forceRefill.setEvent(e -> {
            Man10ShopV3.threadPool.submit(() -> {
                if(!shop.moneyFunction.setVariable(player, "money", getAmount())){
                    return;
                }
