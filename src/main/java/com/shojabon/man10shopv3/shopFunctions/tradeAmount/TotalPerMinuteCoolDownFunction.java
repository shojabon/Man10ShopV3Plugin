...
public class TotalPerMinuteCoolDownFunction extends ShopFunction {
    ...
    public int getTime(){
        return shop.shopData.getJSONObject("totalPerMinuteCoolDown").getInt("time");
    }
    public int getAmount(){
        return shop.shopData.getJSONObject("totalPerMinuteCoolDown").getInt("amount");
    }
    ...
    public AutoScaledMenu getInnerSettingMenu(Player player, Man10ShopV3 plugin){
        ...
        timeSetting.setEvent(ee -> {
            ...
            menu.setOnConfirm(number -> {
                JSONObject request = shop.setVariable(player, "totalPerMinuteCoolDown.time", number);
        ...
        amountSetting.setEvent(ee -> {
            ...
            menu.setOnConfirm(number -> {
                JSONObject request = shop.setVariable(player, "totalPerMinuteCoolDown.amount", number);
                if(!request.getString("status").equals("success")){
                    warn(player, request.getString("message"));
                    return;
                }
